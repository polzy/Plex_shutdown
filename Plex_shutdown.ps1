...
#Popup 
            $Message = "Il semble que personne ne regarde de film sur Plex. Le PC va s'arrêter dans $secondes secondes."
            $Titre = "Plex"
            $btn = 1
            $Icon = 48
...
if ($in -eq "0"){
    ...
    if ($list -like "*playing*"){ 
        ...
        $shutdown = "0"
        if($notif -eq "0"){
            $message =  "Le film " + $list.Movie + " est en cours de lecture par l'utilisateur " + $list.user 
            write-host $message 
            notification
            $notif = "1"
        }
        ...
        write-host "Il semble que personne ne regarde de film sur Plex. Le PC va s'arrêter."
        $notif = "0"
        if($shutdown -eq "0"){
        $shutdown = "1"
    }
    ...
    if ($shutdown -eq "1"){
        ...
        $message = "Il semble que personne ne regarde de film sur Plex. Le PC va s'arrêter a $temp_off"
        write-host $message
        notification
        $shutdown = "2"
    }
